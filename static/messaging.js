...
socket.on('message in', messageParcel => {
  // messages in for other channels are managed by Channel methods
  if (messageParcel.channel == CHANNEL_STATE.name) {

    // emojis are passed to flask as unicode escape sequences
    ...
    document.querySelector("#Chat-Convo").innerHTML += 
      BalloonTemp({
        "classes": messageParcel.author == USER_STATE.username ? "self-chat" : "",
        "author" : messageParcel.author,
        "time"   : new Date().getDate() == date.getDate() ? messageParcel.timeStamp.slice(0,7) :  + messageParcel.timeStamp,
    ...
    // keep channel's participants synced since messages in does not reflect channel updates
    if (!CHANNEL_STATE.participants.includes(messageParcel.author)) {
      CHANNEL_STATE.addUser(messageParcel.author);

      if (!(messageParcel.channel.split("%%").length == 3)) {
        document.querySelector("#Chat-Name button").textContent = CHANNEL_STATE.participants.length;
      }
    }
  // incoming private messages
    // a message in only passes in the message and a channel is needed to reflect to update
    // this does not have much effects given that this is a local copy and next refresh will retrieve the 1st channel info
  } else if (messageParcel.channel.split("%%").length == 3 && 
    !USER_STATE.channels.includes(messageParcel.channel)) {
    // only a local holder
    let channelHolder = new Channel(messageParcel.channel, _, false);
    USER_STATE.addChannel(channelHolder);
    channelHolder.addUser(USER_STATE.username);
    document.querySelector("#Menu-My-Friends-List").append(channelHolder.showChatList(true));
  }
});
...
function sendMessage() {
  ...
  if (message) {
    ...
    if (!USER_STATE.channels.includes(CHANNEL_STATE.name)) { // only for 1st time
      ...
      // After the 1st channel is added, remove the welcome message
      if (USER_STATE.channels.length == 1) {
        MyChannelList.textContent = "";
      }

      // Add in Private list or Channel list
      if (CHANNEL_STATE.name.split("%%").length == 3) {
        document.querySelector("#Menu-My-Friends-List").append(CHANNEL_STATE.showChatList(privateFriendChat));
      } else {
        MyChannelList.append(CHANNEL_STATE.showChatList());
      }
    }
  }
}